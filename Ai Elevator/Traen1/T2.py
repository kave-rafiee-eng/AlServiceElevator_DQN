...
class ElevatorEnv:
    def __init__(self, max_steps=300):
        ...
        self.NumOfFloor = self.sim.NumOfFloor
        self.action_space = type('', (), {})()   # fake action_space
        self.action_space.n = self.NumOfFloor

        self.observation_space = type('', (), {})()
        self.observation_space.n = len(self.sim.get_state())
        self.passenger_scenario = None 
    ...
    def reset(self, seed=None , currentEpisode=0 ):
        ...
        return state, {}   # gym-style output (state, info)

    def step(self, action):
        last_pos = self.sim.CabinPos
        self.sim.move_to_floor(action)
        reward = self.sim.compute_reward(last_pos)

        next_state = self.sim.get_state()
        done = all(p["status"] == PassengerStatus.ARRIVED for p in self.sim.passengers)
...
class DQN_Agent:
    ...
    def __init__(self, env, epsilon_max, epsilon_min, epsilon_decay, 
        ...
        self.main_network = DQN_Network( num_actions=self.action_space_n , input_dim=self.observation_space.n).to(device)
        self.target_network = DQN_Network( num_actions=self.action_space_n , input_dim=self.observation_space.n).to(device).eval()
...
class Model_TrainTest:
    def __init__(self, hyperparams):
        ...
        # Define Env
        self.env = ElevatorEnv( max_steps=self.max_steps )
        """
    ...
    def state_preprocess(self, state, num_states=None):
    ...
    def train(self): 
        ...
        for episode in range(1, self.max_episodes+1):
            state, _ = self.env.reset(seed=seed , currentEpisode=episode )
